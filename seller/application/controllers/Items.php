...
class Items extends MY_Controller{
	...
	public function listing($page = 0){
		if($page == 0){
			...
			if (isset($_SESSION['authuser'])){
				if ($page){
					$products = $this->seller->getAPI('cms','productlist', array(), array('api_hash'=>API_HASH, 'suite' => $this->authuser->suite, 'limit' => 10, 'page' => $page, 'sort' => 'created desc'));
		...
		if($ret['status']){
			// if($this->form_validation->run() != FALSE){
				$data = array(
						...
						'product_name'		=> addslashes($json['name']),
						'product_desc'		=> addslashes($json['desc']),
						...
						'merchant_sla'		=> $json['merchant_sla']
						);
	...
	public function deleteImage(){
		...
		if (isset($_SESSION['authuser'])){
			if (isset($_POST['product_id']) && $_POST['product_id'] != '' && isset($_POST['image_id']) && $_POST['image_id'] != ''){
				$query = $this->seller->getAPI('cms', 'product-image-delete', array(),array('api_hash'=>API_HASH,'suite'=>$this->authuser->suite, 'product_id'=>$_POST['product_id'],'image_id'=>$_POST['image_id']));
